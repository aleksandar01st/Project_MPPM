					...
					case SupportedProfiles.PowerTransformer:
						{
							...
							TransformAndLoadReport report = ProjectImporter.Instance.CreateNMSDelta(concreteModel);

							if (report.Success)
							{
								nmsDelta = ProjectImporter.Instance.NMSDelta;
								success = true;
							}
							else
							{
							...
							ProjectImporter.Instance.Reset();
